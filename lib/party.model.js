...
const { Point, MultiPolygon } = require('mongoose-geojson-schemas');
...
const OPTION_AUTOPOPULATE = ({
  select: { type: 1, name: 1, email: 1, mobile: 1, locale: 1, timezone: 1 },
  maxDepth: POPULATION_MAX_DEPTH
});
...
const PartySchema = new Schema({
  ...
  /**
   * @name location
   * @description A geo-location cetroid of a party operational boundaries.
   *
   * It may be coordinates(longitude and latidude) pair of office reachable by
   * other parties.
   *
   * @type {object}
  ...
  /**
   * @name boundary
   * @description Party geo-operational boundaries.
   *
   * @type {object}
